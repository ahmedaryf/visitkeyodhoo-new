          ...
          data.map((item: any) => (
            <div key={item.id} className='pb-2 pt-1'>
              <button
                ...
                className='text-zinc-900 dark:text-gray-400 text-sm md:text-base px-4 md:px-12 py-3 font-semibold outline w-full text-start rounded-md cursor-pointer body-font'>
                {item.question} {item.name}
              </button>
              <Collapse isOpened={openAccordion === item.id}>
                <div className='text-zinc-900 dark:text-gray-400 text-sm md:text-base prose dark:prose-invert custom-prose text-justify px-4 md:px-12  py-4 rounded-b-md body-font'>
